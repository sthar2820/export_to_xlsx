...
def detect_plant_and_part(sheet):
    plant_row = None
    part_name = None
    plant_found = None
    col_to_search = None
    ...
    # find first SMITCH category row in same column
    if plant_found and col_to_search:
        for row in range(plant_row + 1, min(plant_row + 20, sheet.max_row + 1)):
            val = sheet.cell(row=row, column=col_to_search).value
            if val and str(val).strip().upper() in CATEGORY_LETTERS:
                # Search between plant_row and this row for a part name
                for probe_row in range(plant_row + 1, row):
                    part_val = sheet.cell(row=probe_row, column=col_to_search).value
                    if part_val and isinstance(part_val, str):
                        part_text = part_val.strip()
                        if len(part_text) > 4 and any(char.isdigit() for char in part_text):
                            part_name = part_text
                            break
                break

    return plant_found, part_name
